...
class LineIoManager : public QObject
{
    ...
    Q_PROPERTY(bool dispatcherActive READ dispatcherActive NOTIFY dispatcherActiveChanged) // пока заглушка
...
public:
    ...
    bool fireActive() const { return effectiveFire(); }
    bool fireInput() const { return m_fireInput; }
    bool dispatcherActive() const { return m_dispatcherForce; } // ✅ на будущее
    bool stopActive() const { return m_stopInput; }
...
signals:
    ...
    void dispatcherActiveChanged(bool);
    void stopChanged(bool active);
    void emergencyStop(); // FIRE или STOP пришёл
...
private:
    ...
    // module0 relay bits
    static constexpr int REL_SERV_0 = 0; // управляем (твоя логика) выкл питания на тесте
...
private:
    ...
    bool effectiveFire() const { return m_fireInput || m_forceFire; }

    static inline bool bit(quint8 v, int b) { return (v >> b) & 0x01; }
...
private:
    ...
    void stopStepSwichTimers(){
        if (m_StepSwich) m_StepSwich->stop();
